...
void FeatureIndex::rebuildFeatures(TaggerImpl *tagger) {
  ...
  if (fid == 3662) {
	  printEn = false; /* used for debug */
	  //std::cout << "fid: " << fid << std::endl;
	  //std::cout << "x.size: " << tagger->size() << std::endl;
  }

  path_freelist_[thread_id].free();
  node_freelist_[thread_id].free();
	
  size_t taggerSz = tagger->size();
  for (size_t cur = 0; cur < taggerSz; ++cur) {
	if (printEn){
		std::cout << "fid,cur - node - " <<
				fid << " , " << cur << std::endl;
	}
