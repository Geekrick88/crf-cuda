...
bool runMIRA(const std::vector<TaggerImpl*> &x,
		EncoderFeatureIndex *feature_index, double *alpha, size_t maxitr,
		float C, double eta, unsigned short shrinking_size,
	...
	for (size_t itr = 0; itr < maxitr; ++itr) {
		...
		std::cout << "iter=" << itr << " terr=" << 1.0 * err / all << " serr="
				<< 1.0 * zeroone / x.size() << " act=" << active_set
				<< " uact=" << upper_active_set << " obj=" << obj << " kkt="
				<< max_kkt_violation << std::endl;
...
public:
	...
	void run(cudaFeatures * Features) {
		obj = 0.0;
		err = zeroone = 0;
		std::fill(expected.begin(), expected.end(), 0.0);
		if (Features != 0)
			gpuClearExpected(*Features);
		for (size_t i = start_i; i < size; i += thread_num) {
			double gpu_s = x[i]->setupGPUdata(*Features, &expected[0]);
			obj += (x[i]->gradient(&expected[0]) + gpu_s);
...
bool runCRF(const std::vector<TaggerImpl*> &x,
		EncoderFeatureIndex *feature_index, double *alpha, size_t maxitr,
		float C, double eta, unsigned short shrinking_size,
		unsigned short thread_num, bool orthant, bool cudaEn) {
	...
	/* Initialize GPU's Feature Engine */
	if (cudaEn) {
		Features = (cudaFeatures *)malloc(sizeof(cudaFeatures));
		assert(Features != NULL);

		FeatureCache * feat = feature_index->getFeatureCache();
		size_t cacheSz = feat->size();

		int * base = feat->at(0);
		int * last = feat->at(cacheSz-1);
		/* go to last entry in feature index */
		while(*last != -1) {last++;}

		size_t featSz = ((last - base)+1)*sizeof(int); /*size of full feature array*/

		/* convert feature index alpha features to float */
		Features->cpu.halpha    = new float [feature_index->size()];
		Features->cpu.hexpected = new float [feature_index->size()];
		Features->cpu.halphaSz  = (feature_index->size()) * sizeof(float);


		Features->cpu.hbaseFeat  = base;
		Features->cpu.hfeatureSz = featSz;

		Features->cpu.hcacheSz   = cacheSz * sizeof(int);
		Features->cpu.hfeatures  = (int *)malloc(cacheSz * sizeof(int));
		assert(Features->cpu.hfeatures != NULL);
		for(size_t i = 0; i<cacheSz; i++) {
			int * temp = feat->at(i);
			Features->cpu.hfeatures[i] = (temp - base);
			if (Features->cpu.hfeatures[i] < 0)
				std::cout << " Features->hfeatures[i]: " << Features->cpu.hfeatures[i] << std::endl;
		}

		gpuAllocFeatures(*Features);
		gpuCpyFeatures2GPU(*Features);
	} else {
		Features = 0;
	}
	...
	for (size_t itr = 0; itr < maxitr; ++itr) {
		...
		/* Transfer Alpha vector to the GPU,
		 * --- Until we implement the optimization
		 * step in the GPU. */
		if (cudaEn) {
			std::copy(alpha, alpha + feature_index->size(), Features->cpu.halpha);
			gpuCpyAlpha2GPU(*Features);
		}


		/* This version of CRF does not support threads  */
		for (size_t i = 0; i < thread_num; ++i)
			thread[i].run(Features);
		...
		double diff = (itr == 0 ? 1.0 : std::abs(old_obj - thread[0].obj)
		/ old_obj);
		std::cout << "iter=" << itr << " terr=" << 1.0 * thread[0].err / all
				<< " serr=" << 1.0 * thread[0].zeroone / x.size() << " act="
				<< num_nonzero << " obj=" << thread[0].obj << " diff=" << diff
				<< std::endl;
		...
		if (itr > maxitr || converge == 3)
			break; // 3 is ad-hoc
		if (lbfgs->optimize(feature_index->size(), &alpha[0], thread[0].obj,
...
bool Encoder::convert(const char* textfilename, const char *binaryfilename) {
...
bool Encoder::learn(const char *templfile, const char *trainfile,
		const char *modelfile, bool textmodelfile, size_t maxitr, size_t freq,
		double eta, double C, unsigned short thread_num,
		unsigned short shrinking_size, int algorithm) {
...
#define WHAT_ERROR(msg) do {                                    \
...

	{
		...
		while (ifs) {
			...
			if (algorithm == CUDA) {
				_x->cuda_enabled = true;
			} else {
				_x->cuda_enabled = false;
			}
...
int crfpp_learn(int argc, char **argv) {
	...
	if (param.get<bool> ("help") || (convert && rest.size() != 2) || (!convert
			&& rest.size() != 3)) {
		std::cout << param.help();
		return 0;
	}
	...
	const unsigned short shrinking_size = param.get<unsigned short> (
			"shrinking-size");
	...
	if (salgo == "crf" || salgo == "crf-l2") {
		...
		std::cout << "cuda mode selected" << std::endl;
		algorithm = CRFPP::Encoder::CUDA;
		//cuda_hello(argc, argv);
		//return -1;
	} else {
	...
	if (convert) {
		...
		if (!encoder.learn(rest[0].c_str(), rest[1].c_str(), rest[2].c_str(),
				textmodel, maxiter, freq, eta, C, thread, shrinking_size,
				algorithm)) {
			std::cerr << encoder.what() << std::endl;
			return -1;
		}
